...
describe('Notifications component testing', () => {
    ...
    test('UL list present in render', () => {
        const { getByText } = render(<Notify displayDrawer={true} type="default" value="testByJBA" />);
        const ulElement = getByText(/testByJBA/i);
        expect(ulElement).toBeInTheDocument();
    });

    test('Dangerouse HTML set in render', () => {
        const { getByText } = render(<Notify displayDrawer={true} type="default" html={{__html: "<u>testJBA</u>"}} />);
